"""Provincial Tax Scraper.

This scraper crawls provincial tax resources.
It's designed to be self-contained and easily replicable for other scrapers.

Usage:
    python -m tax_rag_scraper.active-crawlers.provtax_scraper
"""
...
# Start URLs for provincial tax crawl
START_URLS = [
    'https://www.ontario.ca/page/benefits-and-taxes',
    'https://www2.gov.bc.ca/gov/content/taxes',
    'https://www.alberta.ca/about-taxes-in-alberta',
    'https://www.gov.mb.ca/finance/taxation/index.html',
    'https://www2.gnb.ca/content/gnb/en/departments/finance/taxes.html',
    'https://www.novascotia.ca/topic/money-tax-and-benefits',
    'https://www.princeedwardisland.ca/en/topic/tax-information',
    'https://www.revenuquebec.ca/en/citizens/',
    'https://www.revenuquebec.ca/en/businesses/',
    'https://www.saskatchewan.ca/business/taxes-licensing-and-reporting/provincial-taxes-policies-and-bulletins',
    'https://www.saskatchewan.ca/business/taxes-licensing-and-reporting/tax-incentives',
    'https://www.gov.nl.ca/fin/tax-programs-incentives/business/',
...
async def main() -> None:
    """Run the Provincial Tax scraper."""
    ...
    if not START_URLS or START_URLS[0].startswith('https://example.com'):
        ...
        logger.error('\nPlease update the START_URLS in provtax_scraper.py')
        logger.error('Add the target URLs for provincial tax resources')
        sys.exit(1)
    ...
    logger.info('\n[OK] Provincial Tax scraper complete.')
